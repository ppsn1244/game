...
function veigAttack(){
	...
	if (attackChoice == 1){
		...
		if (hitChance <=5){
			...
			bottomRow.innerHTML += "<br>Monster hit with you a basic Blast, doing " + dmg + " damage. You now have " + megaHP + "hp.";
			var megaHPBarWidth = (megaHP/100)*300;
			megaManHP.style.width = megaHPBarWidth + "px";
		} else {
			bottomRow.innerHTML += "<br>Monster missed!";
		}
	} else {
		var hitChance = Math.round(Math.random()*10);
		if (hitChance <=3){
...
function blast(){
	...
	if (hitChance <=7){
		...
		bottomRow.innerHTML = "You hit Battle Boss Monster with your blast, doing " + dmg + " damage. Monster now has " + veigarHP + "hp.";
